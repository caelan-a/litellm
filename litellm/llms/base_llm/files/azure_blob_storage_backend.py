...
class AzureBlobStorageBackend(BaseFileStorageBackend, AzureBlobStorageLogger):
    ...
    def __init__(self, **kwargs):
        ...
        # Skip the periodic flush task since we're using this for file storage, not logging
        AzureBlobStorageLogger.__init__(self, **kwargs)
        
        # Stop the periodic flush task if it was started
        # (We don't need it for file storage)
        if hasattr(self, 'periodic_flush_task'):
            try:
                self.periodic_flush_task.cancel()
            except Exception:
                pass
